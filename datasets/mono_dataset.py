...
class MonoDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if os.path.exists(imu_path):
            ...
            imu = np.zeros((IMU_FREQ * (frame_index + 2) + 1, 6), dtype=np.float32)
        start = frame_index * IMU_FREQ
        inputs[("imu")] = imu[start:start + IMU_FREQ + 1]
        start_next = (frame_index + 1) * IMU_FREQ
        inputs[("imu1")] = imu[start_next:start_next + IMU_FREQ + 1]
